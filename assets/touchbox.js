    ...
    TouchBox.prototype = {
        ...
        to: function (toIndex, silent, /* private */ isTouch) {
            ...
            if (toIndex > -1 && toIndex <= last && toIndex != active && me.ee.fireEvent("beforeslide", toIndex, active) !== false) {
                ...
                isSlideDown =
                    last > 1
                        ? (toIndex < active && active < last) || (toIndex == last - 1 && active == last) || (toIndex == last && active === 0)
                        : active > toIndex;
                slideFn(isSlideDown);
            } else {
        ...
        slide: function (fromIndex, toIndex, isSlideDown, silent) {
            ...
            duration = silent
                ? 0
                : animation.duration
                ? animation.duration.call(me, fromEl, toEl, fromIndex, toIndex, isSlideDown)
                : Math.round((Math.abs(translateY) / offsetHeight) * baseDuration);
